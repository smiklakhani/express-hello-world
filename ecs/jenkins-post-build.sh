...
#TASK_FILE=ecs/tasks/express-task-v_${VERSION}.json
TASK_FILE=ecs/tasks/express-task.json
sed -i "s;%BUILD_NUMBER%;${VERSION};g" ecs/tasks/express-task.json
#sed -e "s;%BUILD_NUMBER%;${VERSION};g" ecs/tasks/express-task.json > $TASK_FILE

# Replace the desired count
SERVICE_FILE="ecs/services/express-service.json"
sed -i "s;%DESIRED_COUNT;${COUNT};g" ecs/services/express-service.json
